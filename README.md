# Author's Assistant

Author's Assistant is a local-first, Markdown-first writing and knowledge-management project.

The project helps an author collect research, organize knowledge, draft content, and run AI-assisted revision workflows while keeping final editorial judgment human.

## Start Here

- [Documentation Landing Page](docs/index.md)
- [Project Requirements](docs/PROJECT_REQUIREMENTS.md)
- [Architecture](docs/ARCHITECTURE.md)
- [Copilot Context](docs/COPILOT_CONTEXT.md)
- [Writing Style](docs/WRITING_STYLE.md)
- [Editing Rules](docs/EDITING_RULES.md)

## Core Principles

- Local-first workflows
- Markdown as source of truth
- Prompt assets separated from application code
- Incremental, maintainable tooling

## Feature Roadmap

- [Roadmap](docs/features/000-ROADMAP.md)
- [Personal Knowledge Base](docs/features/001-PERSONAL-KNOWLEDGE-BASE.md)
- [Knowledge Harvesting Workflow](docs/features/002-KNOWLEDGE-HARVESTING-WORKFLOW.md)

## Repository

- [GitHub Repository](https://github.com/paramireze-afk/authors-assistant)
